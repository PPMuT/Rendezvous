...
class CallGraph(CFGBase):
    ...
    def get_pre_sequence_call_graph(self, start_node, tree_nodes, pre_sequence_nodes,cut=False,binary_cfg_info_path=""):
        ...
        if cut and binary_cfg_info_path:
            basedir = binary_cfg_info_path.split("data")[0]
            typeIIsourceDir = os.path.join(basedir, "dataflow", "procedures", "typeIIsource")
            typeIsourceDir = os.path.join(basedir, "dataflow", "procedures", "typeIsource")
            typeIIsources = os.listdir(typeIIsourceDir)
            typeIsources = os.listdir(typeIsourceDir)
            if "__pycache__" in typeIIsources:
                typeIIsources.remove("__pycache__")
            if "__init__.py" in typeIIsources:
                typeIIsources.remove("__init__.py")
            if "__pycache__" in typeIsources:
                typeIsources.remove("__pycache__")
            if "__init__.py" in typeIsources:
                typeIsources.remove("__init__.py")

            with open(binary_cfg_info_path, 'r', encoding='utf-8') as f:
                cfg_data = json.load(f)

            allSourceFunc = set()
            for file in set(typeIsources).union(typeIIsources):
                allSourceFunc.add(file[:-3])
            for addr in cfg_data:
                function_name = cfg_data[addr]["name"]
                if function_name.startswith("sub_"):
                    function_name = function_name.lower()
                if function_name in allSourceFunc:
                    allSourceFunc.remove(function_name)
                    allSourceFunc.add(addr)
            visited = set()
            queue = deque()
            queue.append(hex(start_node.addr)[2:])
            true_start = ""
            while queue:
                ...
                if current in visited:
                    continue
                visited.add(current)
                node = cfg_data.get(current)

                for call in node.get("call", []):
                    callee = call[2]
                    if isinstance(callee, int):
                        callee_hex = hex(callee)[2:]
                        if callee_hex in allSourceFunc:
                            true_start = current
                        if callee_hex not in visited:
                            queue.append(callee_hex)
                            visited.add(callee_hex)
                    else:
                        if callee in allSourceFunc:
                            true_start = current
                    if true_start:
                        final_worklist = []
                        found = False
                        for item in worklist:
                            if found:
                                final_worklist.append(item)
                                continue
                            if hex(item.addr)[2:] == true_start:
                                found = True
                    return final_worklist
