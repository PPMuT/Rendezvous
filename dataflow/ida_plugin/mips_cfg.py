        ...
        while i <= end:
            ...
            if is_code_region(Dword):
                if Dword in functions:
                    ...
                    mnem = idc.print_insn_mnem(Dword)
                    opnd0_name = idc.print_operand(Dword, 0)
...
def recognise_functions_v2(functions):
    ...
    if arch_bits == 32:
        ...
        print("64-bits ARM not complete recognise_function_gcc64")
...
def get_block_info_mips(funcea, block, functions, function_names, func_info, blocks_record, xref_funcptrs, callees):
    ...
    while ea != BADADDR and ea < block_end:
        ...
        mnem = idc.print_insn_mnem(ea)
        # 7.5 t9 the second parameter of jalr but the first of jr
        opnd0_name = idc.print_operand(ea, 0)
        opnd1_name = idc.print_operand(ea, 1)
        # print(mnem, opnd0_name)
        ins = GetDisasm(ea)
        call_mnems = ['jalr', 'jr', 'j', 'jal']
        if mnem in call_mnems and (opnd0_name == '$t9' or opnd1_name == '$t9'):
            # print("Call: 0x%x %s" % (ea, ins))
            ins_list = ins.split(" ;")
            ins_list = list(filter(None, re.split(r'[; \s]\s*', ins)))
            # print(ins_list)
            if len(ins_list) >= 3 and (ins_list[1] == opnd0_name or ins_list[1] == opnd1_name):
            ...
            opnd0_value = idc.get_operand_value(ea, 0)
            addr = idc.get_func_attr(opnd0_value, 0)
            if addr != BADADDR and addr != funcea:
                attr = idc.get_func_attr(opnd0_value, 8)
                if attr == 0xc0:
                    func_name = idc.get_func_name(opnd0_value)
                    func_info['call'].append((block_start, ea, func_name))
                    # print("  Lib function: %s" % (func_name))
                else:
...
def create_sections(code_start, code_end):
    ...
    for segment in segments:
        ...
        print("start and end: (%x %x)" % (idc.get_segm_start(segment), idc.get_segm_end(segment)))
...
def generate_cfg_mips(funcs, function_names, cfg_record, blocks_record, switch_record):
    ...
    for funcea in funcs:
        ...
        for block in flow:
            ...
            #     print(" pre block: 0x%x" % (pre_block.start_ea))
            # if len(pre_blocks) == 0 and block.start_ea != funcea:
            #     print(" Un-link block: 0x%x" % (block.start_ea))

            if len(succ_blocks) == 0 and block.end_ea != function_obj.end_ea:
                # print(" Exit-block: 0x%x %x %x" % (block.start_ea, block.end_ea, function_obj.end_ea))
        ...
        #     print("exit-block: %x %x" % (exit_block.start_ea, exit_block.end_ea))
