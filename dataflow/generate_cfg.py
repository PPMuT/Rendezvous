...
class FunctionCFG(CFGBase):
    ...
    def generate_function_cfg(self, function, start_ida_blocks):
        ...
        while stack:
            ...
            # === 【新增补丁：如果当前块翻译失败，直接跳过】 ===
            if not ida_block.contain_blocks:
                print(f"Warning: Skipping empty source block at {ida_block}")
                continue
            # ===============================================
            ...
            if jumpkind == 'call' and len(out_edges) == 0:
                ...
                if arch_flag and jmp_block:
                    ...
                    for _, succ_ida_block in out_edges:
                        # === 【开始修改】 ===
                        # 原代码直接取 [0]，我们要加个判断
                        if not succ_ida_block.contain_blocks:
                            print(f"Warning: Skipping empty block at {succ_ida_block}") 
                            continue 
                        # === 【结束修改】 ===
